...
apiMocker.loadConfigFile = function() {
	if (apiMocker.configFilePath) {
		...
		if (process.env.VCAP_APP_PORT) {
			...
			apiMocker.log('********** FOUND VCAP_APP_PORT, setting to : '+ process.env.VCAP_APP_PORT);
			configJson.port = process.env.VCAP_APP_PORT;
		}
        else if (process.env.PORT) {
            apiMocker.log('********** NO VCAP_APP_PORT but found PORT, setting to : '+ process.env.PORT);
            configJson.port = process.env.PORT;
        }
		else{
            apiMocker.log('********** NO VCAP_APP_PORT FOUND');
		}
